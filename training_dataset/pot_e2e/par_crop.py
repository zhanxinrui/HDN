...
def crop_video(video_set, video, crop_path, instanc_size):
    ...
    for idx, img_path in enumerate(jpgs):
        if idx==0 or idx%2==1:
            ...
            bbox = [int(min_p[0]), int(min_p[1]),int(max_p[0]), int(max_p[1]),]   # (xmin, ymin, xmax, ymax)
            im = cv2.imread(img_path)
            avg_chans = np.mean(im, axis=(0, 1))

            z, x = crop_like_SiamFC(im, bbox, video, instanc_size=instanc_size, padding=avg_chans)
            cv2.imwrite(join(video_crop_base_path, '{:06d}.{:02d}.z.jpg'.format(int(avail_idx), 0)), z)
